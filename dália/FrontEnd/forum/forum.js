class Posts {
  constructor(usuario_id, titulo, conteudo) {
    this.usuario_id = usuario_id; // Utilize o id do usuário passado no construtor
    this.titulo = titulo;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadPosts = async () => {
    try {
      ...
      console.log(posts); // Adicione isso para verificar os dados retornados

      for (const post of posts) {
        const postElement = await createPostElement(
          post.titulo || 'Título Indefinido', // Verifique se o título não é undefined
          post.conteudo || 'Conteúdo Indefinido', // Verifique se o conteúdo não é undefined
          post.likes || 0, // Garanta que likes seja um número
          post.id // Certifique-se de que o ID está correto
        );
  ...
  const createPostElement = async (title, content, likes, postId) => {
    ...
    // Carregando e exibindo comentários
    const comments = await loadComments(postId); // Aguarde a chamada aqui
    ...
    return postElement; // Retorne o postElement para ser adicionado ao container
  };
  ...
  loadPosts(); // Carrega os posts ao inicializar a página
});
